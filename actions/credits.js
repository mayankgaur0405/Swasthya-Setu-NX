...
/* this page is used to manage user credits for booking appointments
 * It checks the user's subscription plan and allocates monthly credits accordingly.
 * It also deducts credits when a user books an appointment with a doctor.
 * The credit system is designed to ensure that users have sufficient credits to book appointments,
 * and that doctors receive credits for their services.
 * The code handles different subscription plans (free, standard, premium) and allocates credits accordingly
 * It also manages credit transactions, ensuring that credits are deducted from the patient's account
 * and added to the doctor's account when an appointment is booked.
 */
...
const APPOINTMENT_CREDIT_COST = 2; //1 appointments is done via spending 2 credits
...
export async function checkAndAllocateCredits(user) {
  try {
    ...
    //user has transations check databse prisma file for that
