...
export async function bookAppointment(formData) {
  ...
  try {
    ...
    // Deduct credits from patient and add to doctor as the appointment has booked
    const { success, error } = await deductCreditsForAppointment( //this is an function which we imported 
      patient.id,
      doctor.id
    );
...
async function createVideoSession() {
  try {
    //mediamode routed basically means that all the server will routed through all the media trafiic will routed through vonage servers
...
export async function generateVideoToken(formData) {
  ...
  try {
    ...
    const connectionData = JSON.stringify({ // stringfy change the JS object to Json string
      name: user.name,
      role: user.role,
      userId: user.id,
    });
 ...
 * //=> Tue Sep 02 2014 23:59:59.999 */
    ...
    for (const day of days) {
      ...
      //actually this is ned to be done as when doctor has selcted his available time status there might be a different date from now 
      // so we need to change this date, month , year (not time) as per the current date status
      ...
      availabilityStart.setFullYear(
        day.getFullYear(),
        day.getMonth(),
        day.getDate() //changes the date which had been set earlier and now updating it to current notices here we did not change in time
      );
      ...
      //we iterate with 30 minutes schdeules before the end or equal the end time which we have 
      while (
