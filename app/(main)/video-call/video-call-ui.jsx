...
export default function VideoCall({ sessionId, token }) {
  ...
  const handleScriptLoad = () => {
    ...
    //check if Vobage Ot(OpenTok) is available
    if (!window.OT) {
...
/*       OT is the Vonage global object from their JS SDK
initSession(appId, sessionId) initializes a new video session object
We store it in a ref to persist across re-renders */

      sessionRef.current = window.OT.initSession(appId, sessionId);

      // Subscribe to new streams
   /*    Whenever another user joins and starts publishing, a new streamCreated event fires
You subscribe to that stream, so you can see their video
It renders into the DOM element with id="subscriber"
 */
      sessionRef.current.on("streamCreated", (event) => {
        ...
        sessionRef.current.subscribe(
          event.stream,
          "subscriber", //It renders into the DOM element with id="subscriber"
          {
            insertMode: "append",
            width: "100%",
            height: "100%",
      ...
      sessionRef.current.on("sessionConnected", () => {
       ...
       insertmode - append 
        ...
        without removing anything else that might be there.

        insert mode - Replace
        It would wipe all contents inside the div#subscriber and replace them with just the video element. 
        So if you had a label, heading, icon, etc., inside that div, it would be lost.
 */
      ...
      sessionRef.current.connect(token, (error) => {
        if (error) {
          ...
          if (publisherRef.current) {
            // Publishing allows others in the session to subscribe to you
