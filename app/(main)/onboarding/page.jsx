...
import { useState } from "react"; // USED THIS
import { useRouter } from "next/navigation";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod"; // WE USE ZOD RESOLVER FOR VALIDATION
import {
  Card,
  CardContent,
  CardDescription,
...
import { doctorFormSchema } from "@/lib/schema"; //CHECK THIS
import { SPECIALTIES } from "@/lib/specialities";
import useFetch from "@/hooks/use-fetch"; //CHECK THIS
...
/*   You’re telling react-hook-form:
  ...
  const {
    ...
    formState: { errors },
    setValue,
    watch, //Reads current value of one or more fields in real-time.
  } = useForm({
    //     Integrates Zod schema validation with React Hook Form.
    // All form fields are validated using the Zod schema instead of manually writing rules.
  ...
  useEffect(() => {
    if (data && data?.success) {
      router.push(data.redirect); //Navigate to a route programmatically
    }
  }, [data]);
  ...
  if (step === "choose-role") {
    return (
      ...
      {/* FOR DOCTOR selection and when selcting than setnest step for getting doctor details */}
        <Card
          className="border-emerald-900/20 hover:border-emerald-700/40 cursor-pointer transition-all"
