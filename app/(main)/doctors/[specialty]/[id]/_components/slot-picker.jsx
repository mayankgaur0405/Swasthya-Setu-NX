...
export function SlotPicker({ days, onSelectSlot }) {
  ...
  // returns the first day where day.slots.length > 0 (i.e., slots are available).
  ...
  const confirmSelection = () => {
    if (selectedSlot) {
      onSelectSlot(selectedSlot); // function from props
    }
  };
  ...
  return (
    <div className="space-y-6">
      <Tabs
        ...
        // onValueChange={(value) => setActiveTab(value)}  this is equivalent to the above line
        className="w-full"
      >
        {/* overflow-x-auto : Makes the element horizontally scrollable if the content overflows its width. */}
        {/* users can scroll left/right. */}
            ...
            >
              <div className="flex gap-2">
                <div className=" opacity-80">
                  {/* formats the date like: "Jun 21" (for June 21) */}
                  {format(new Date(day.date), "MMM d")}
                </div>
                {/* returns the abbreviated day name: "Fri" for Friday, "Mon" for Monday */}
